...
gulp.task('connect', ['lab'], function(){
  ...
  gulp.watch(path.resolve(paths().source.sass, '**/*.{sass,scss}'), ['sass:style']);
...
gulp.task('sass:style', function(){
  return gulp.src(path.resolve(paths().source.sass + '*.{sass,scss}'))
         .pipe(sass({
           indentedSyntax: true,
...
gulp.task('assets', ['cp:js', 'cp:img', 'cp:font', 'cp:data', 'sass:style', 'cp:styleguide' ]);
